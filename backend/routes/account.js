...
accountRouter.get('/deletion-check', async (req, res, next) => {
  try {
    return res.json({ blockingTrees: await getTreesOwnedByUser(req.user.id) });
  } catch (error) {
    return next(error);
  }
});

accountRouter.post('/trees/:id/transfer-ownership', requireTreeRole(['owner']), async (req, res, next) => {
  try {
    ...
    await transferTreeOwnership(treeId, req.user.id, toUserId);
...
accountRouter.delete('/', async (req, res, next) => {
  try {
    const blockingTrees = await getTreesOwnedByUser(req.user.id);
