...
//     "husband's mother" -> "mother-in-law"), rather than being spelled out
//     as two separate hops that just restate what classifyPath's `short`
//     label already says. Only fires at position 0 or the last position
//     (mirroring classifyPath's own leading/trailing spouse stripping), and
//     only once up front - so the `inLaw` flag it sets is in place before
//     the collapses below can propagate it further inward (e.g.
//     grandmother-in-law).
 ...
 *
 * @param {string|number} rootUserId - the "from" person (Person A - either the
 *   signed-in member's focused tree node, or any person chosen in the finder)
...
export function getRelationshipPath(rootUserId, targetUserId, familyData) {
  ...
  // Visual node chain (Person A -> ... -> Person B) for the step-by-step
  // path display - built from the same reduceHops() collapsing as the text
  // chain above, so the visual and plain-language descriptions never disagree.
